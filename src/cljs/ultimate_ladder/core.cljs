...
(defn challenge-player [id name]
  (let [confirm-challenge (js/confirm (str "Do you want to challenge " name "?"))]
    (if confirm-challenge
      (go (let [response (<! (http/post "/challenge" {:edn-params {:challenger (:id @player-self) :challengee id}}))]
          (prn response)
          (prn (:body response))
...
(defn challenge-win [id]
  (go (let [response (<! (http/post "/confirm-result" {:form-params {:player (:id @player-self) :opponent id :result :win}}))]
...
(defn challenge-loss [id]
  (go (let [response (<! (http/post "/confirm-result" {:form-params {:player (:id @player-self) :opponent id :result :loss}}))]
      (reset! player-store (get-in response [:body :players]))
      (prn (get-in response [:body :challenges (:id @player-self)]))
      (swap! player-self assoc :challenge (get-in response [:body :challenges (:id @player-self)])))))

(defn refresh []
  (go (let [response (<! (http/get "/ladder"))]
      (prn response)
      (prn (:body response))
      (reset! player-store (get-in response [:body :players]))
      (swap! player-self assoc :challenge (get-in response [:body :challenges (:id @player-self)])))))
